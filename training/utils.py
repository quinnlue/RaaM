...
class ProgressBarManager:
    ...
    def update_progress(self, loss_value: float, optimizer, pbar, scheduler=None) -> None:
        self.current_step += 1
...
class Metrics:
    def __init__(self, num_epochs, dataloader_length, log_frequency=100,
        ...
        self.logger = setup_logger('training_metrics', training_log_path, mode='a', header="step,loss") if training_log_path else None
        
        self.row_buffer = []
        self.pbar_manager = ProgressBarManager(self.total_steps, time.perf_counter())

    def update(self, loss, optimizer, pbar, scheduler=None):
        self.pbar_manager.update_progress(loss, optimizer, pbar, scheduler)
        ...
        if self.pbar_manager.current_step % self.log_frequency == 0:
            if self.row_buffer and self.logger:
                for row in self.row_buffer:
                    self.logger.info(f"{row['step']},{row['loss']:.6f}")
